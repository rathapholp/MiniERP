...
@Component({
  ...
  template: `
    <app-page-header title="Receivables"></app-page-header>
    <mat-card>
      <mat-card-content>
...
export class ReceivablesComponent implements OnInit {
  ...
  ngOnInit() {
    this.api.get<any[]>('reports/receivables').subscribe(d => {
      this.items = d;
      this.total = d.reduce((s, i) => s + (Number(i.total) - Number(i.paidAmount)), 0);
    });
  }
}
