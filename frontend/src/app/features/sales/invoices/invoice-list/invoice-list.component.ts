...
@Component({
  ...
  template: `
    <app-page-header title="Invoices" newLink="/sales/invoices/new" newLabel="New Invoice"></app-page-header>
    <mat-card>
      <mat-card-content>
        <div class="filters">
        ...
        <table mat-table [dataSource]="items">
          <ng-container matColumnDef="number"><th mat-header-cell *matHeaderCellDef>Number</th><td mat-cell *matCellDef="let r"><a [routerLink]="['/sales/invoices', r.id]">{{ r.number }}</a></td></ng-container>
          ...
          <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th>
            <td mat-cell *matCellDef="let r" class="mat-column-actions">
              <a mat-icon-button [routerLink]="['/sales/invoices', r.id]"><mat-icon>visibility</mat-icon></a>
            </td>
          </ng-container>
...
export class InvoiceListComponent implements OnInit {
  items: Invoice[] = [];
  columns = ['number', 'date', 'dueDate', 'contact', 'total', 'paid', 'status', 'actions'];
  search = new FormControl('');
  statusFilter = new FormControl('');
  statuses = ['DRAFT', 'SENT', 'PARTIAL', 'PAID', 'OVERDUE', 'CANCELLED', 'VOID'];
  ...
  load() { this.api.get<Invoice[]>('invoices', { search: this.search.value, status: this.statusFilter.value }).subscribe(d => this.items = d); }
}
