...
@Component({
  ...
  template: `
    <app-page-header title="Bills" newLink="/purchases/bills/new" newLabel="New Bill"></app-page-header>
    <mat-card>
      <mat-card-content>
        <div class="filters">
        ...
        <table mat-table [dataSource]="items">
          <ng-container matColumnDef="number"><th mat-header-cell *matHeaderCellDef>Number</th><td mat-cell *matCellDef="let r"><a [routerLink]="['/purchases/bills', r.id]">{{ r.number }}</a></td></ng-container>
          ...
          <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th>
            <td mat-cell *matCellDef="let r"><a mat-icon-button [routerLink]="['/purchases/bills', r.id]"><mat-icon>visibility</mat-icon></a></td>
          </ng-container>
...
export class BillListComponent implements OnInit {
  items: Bill[] = [];
  columns = ['number', 'date', 'contact', 'total', 'paid', 'status', 'actions'];
  ...
  load() { this.api.get<Bill[]>('bills', { search: this.search.value, status: this.statusFilter.value }).subscribe(d => this.items = d); }
}
