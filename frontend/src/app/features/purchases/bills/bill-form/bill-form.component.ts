...
@Component({
  ...
  template: `
    <app-page-header title="New Bill"></app-page-header>
    <mat-card>
      <mat-card-content>
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          ...
          <mat-form-field appearance="outline" class="full-width"><mat-label>Note</mat-label><textarea matInput formControlName="note" rows="2"></textarea></mat-form-field>
          <app-document-line-editor [linesArray]="linesArray" (totalsChanged)="onTotals($event)"></app-document-line-editor>
          <div class="form-actions">
            <button mat-button type="button" routerLink="/purchases/bills">Cancel</button>
            <button mat-raised-button color="primary" type="submit" [disabled]="form.invalid || linesArray.length === 0">Create Bill</button>
          </div>
        </form>
      </mat-card-content>
    </mat-card>
...
export class BillFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.api.post('bills', payload).subscribe({ next: () => { this.snack.open('Bill created', 'OK', { duration: 2000 }); this.router.navigate(['/purchases/bills']); }, error: (e) => this.snack.open(e.error?.message || 'Error', 'OK', { duration: 3000 }) });
  }
}
