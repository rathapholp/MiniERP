...
@Component({
  ...
  template: `
    <app-page-header title="New Purchase Order"></app-page-header>
    <mat-card>
      <mat-card-content>
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <div class="form-row">
            ...
            <mat-form-field appearance="outline">
              ...
              <input matInput [matDatepicker]="dp2" formControlName="expectedDate">
          ...
          <div class="form-actions">
            <button mat-button type="button" routerLink="/purchases/orders">Cancel</button>
            <button mat-raised-button color="primary" type="submit" [disabled]="form.invalid || linesArray.length === 0">Create PO</button>
          </div>
        </form>
      </mat-card-content>
    </mat-card>
...
export class PoFormComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private api: ApiService, private router: Router, private snack: MatSnackBar) {
    ...
    this.form = this.fb.group({ contactId: [null, Validators.required], date: [new Date(), Validators.required], expectedDate: [null], note: [''] });
  }

  ngOnInit() { this.api.get<Contact[]>('contacts', { type: 'SUPPLIER' }).subscribe(c => this.contacts = c); }
  onTotals(t: any) {}
  ...
  onSubmit() {
    ...
    const payload = { ...this.form.value, date: this.form.value.date?.toISOString?.() ?? this.form.value.date, expectedDate: this.form.value.expectedDate?.toISOString?.() ?? this.form.value.expectedDate, lines };
    this.api.post('purchase-orders', payload).subscribe({ next: () => { this.snack.open('PO created', 'OK', { duration: 2000 }); this.router.navigate(['/purchases/orders']); }, error: (e) => this.snack.open(e.error?.message || 'Error', 'OK', { duration: 3000 }) });
  }
}
